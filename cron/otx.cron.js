...
async function enumeration() {
    ...
    targets.forEach((t) => {
        ...
        scopes.forEach((scope) => {
            console.log(`curl -s "https://otx.alienvault.com/api/v1/indicators/domain/${scope}/passive_dns" | jq -r '.passive_dns[].hostname' ${out_of_scopes}`)
            exec(`curl -s "https://otx.alienvault.com/api/v1/indicators/domain/${scope}/passive_dns" | jq -r '.passive_dns[].hostname' | grep "\.${scope}" ${out_of_scopes}`, async (error, stdout, stderr) => {
                if (error) {
